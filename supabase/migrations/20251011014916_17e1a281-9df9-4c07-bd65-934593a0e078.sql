-- Add geolocation columns to url_events table
ALTER TABLE public.url_events 
ADD COLUMN IF NOT EXISTS latitude NUMERIC(10, 7),
ADD COLUMN IF NOT EXISTS longitude NUMERIC(10, 7),
ADD COLUMN IF NOT EXISTS city TEXT,
ADD COLUMN IF NOT EXISTS region TEXT,
ADD COLUMN IF NOT EXISTS country TEXT,
ADD COLUMN IF NOT EXISTS country_code TEXT,
ADD COLUMN IF NOT EXISTS zip_code TEXT;

-- Add index for location-based queries
CREATE INDEX IF NOT EXISTS idx_url_events_location ON public.url_events(country, region, city);

-- Update log_event function to accept location parameters
CREATE OR REPLACE FUNCTION public.log_event(
  _token text,
  _event_type text,
  _utm_snapshot jsonb DEFAULT NULL,
  _ip_address inet DEFAULT NULL,
  _user_agent text DEFAULT NULL,
  _latitude numeric DEFAULT NULL,
  _longitude numeric DEFAULT NULL,
  _city text DEFAULT NULL,
  _region text DEFAULT NULL,
  _country text DEFAULT NULL,
  _country_code text DEFAULT NULL,
  _zip_code text DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
DECLARE
  _event_id UUID;
  _token_level INTEGER;
  _recent_event_count INTEGER;
  _rate_limit_window INTERVAL := '1 minute';
  _max_events_per_window INTEGER := 10;
BEGIN
  -- Validate event type
  IF _event_type NOT IN ('scan', 'view', 'share') THEN
    RAISE EXCEPTION 'Invalid event_type: must be scan, view, or share';
  END IF;
  
  -- Get token level (returns NULL if token doesn't exist)
  SELECT level INTO _token_level
  FROM public.tokens
  WHERE token = _token;
  
  IF _token_level IS NULL THEN
    RAISE EXCEPTION 'Token not found: %', _token;
  END IF;
  
  -- Rate limiting ONLY for L01-L03 tokens (level > 0)
  IF _token_level > 0 AND _ip_address IS NOT NULL THEN
    -- Count recent events from this IP for this token
    SELECT COUNT(*) INTO _recent_event_count
    FROM public.url_events
    WHERE token = _token
      AND ip_address = _ip_address
      AND occurred_at > (now() - _rate_limit_window);
    
    -- Reject if over limit
    IF _recent_event_count >= _max_events_per_window THEN
      RAISE EXCEPTION 'Rate limit exceeded: max % events per % for level % tokens (IP: %)', 
        _max_events_per_window, 
        _rate_limit_window,
        _token_level,
        _ip_address;
    END IF;
  END IF;
  
  -- Insert event with location data
  INSERT INTO public.url_events (
    token,
    event_type,
    utm_snapshot,
    ip_address,
    user_agent,
    latitude,
    longitude,
    city,
    region,
    country,
    country_code,
    zip_code,
    occurred_at
  ) VALUES (
    _token,
    _event_type,
    _utm_snapshot,
    _ip_address,
    _user_agent,
    _latitude,
    _longitude,
    _city,
    _region,
    _country,
    _country_code,
    _zip_code,
    now()
  )
  RETURNING id INTO _event_id;
  
  RETURN _event_id;
END;
$$;