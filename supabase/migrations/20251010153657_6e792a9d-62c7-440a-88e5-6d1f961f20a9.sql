-- Drop and recreate both functions with updated domain

DROP FUNCTION IF EXISTS mint_l00(uuid, text, text, text);
DROP FUNCTION IF EXISTS mint_share(text, text);

-- Recreate mint_l00 with new domain
CREATE OR REPLACE FUNCTION mint_l00(_eoa_id uuid, _deck_slug text, _utm_medium text DEFAULT 'qr'::text, _utm_content text DEFAULT NULL::text)
...
BEGIN
  ...
  -- Build full URL with updated domain
  _full_url := format(
    'https://solidarity-slideshow.lovable.app/deck/%s?utm_campaign=%s&utm_id=%s&utm_source=%s&utm_medium=%s&utm_content=%s&t=%s&v_lvl=00',
    _deck_slug,
    _utm_campaign,
    _utm_id,
    _utm_source,
...
-- Recreate mint_share with new domain
CREATE OR REPLACE FUNCTION mint_share(_parent_token text, _utm_medium text)
...
BEGIN
  ...
  -- Build full URL with updated domain
  _full_url := format(
    'https://solidarity-slideshow.lovable.app/deck/%s?utm_campaign=%s&utm_id=%s&utm_source=%s&utm_medium=%s&t=%s&p=%s&v_lvl=%s',
    _deck_slug,
    _utm_campaign,
    _utm_id,
    _utm_source,
...
-- Update existing tokens to use new domain
UPDATE tokens 
SET full_url = REPLACE(full_url, 'https://app.democracyforge.org', 'https://solidarity-slideshow.lovable.app')
WHERE full_url LIKE 'https://app.democracyforge.org%';