-- Update mint_l00 function to allow overwriting existing L00 tokens
...
BEGIN
  ...
  -- Remove the check for existing L00 token - allow overwriting
  ...
  -- If token exists, delete it and all its children before creating new one
  IF _token_exists THEN
    DELETE FROM public.tokens WHERE token = _token OR root_token = _token;
  END IF;
