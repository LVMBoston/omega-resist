...
DROP FUNCTION IF EXISTS public.get_campaign_stats(text[]);

CREATE OR REPLACE FUNCTION public.get_campaign_stats(campaign_codes text[])
 RETURNS TABLE(campaign_code text, earliest_active timestamp with time zone, latest_active timestamp with time zone, real_data_rows bigint, sim_data_rows bigint, l0_count bigint, l1_count bigint, l2_count bigint, l3_count bigint, l3_plus_count bigint)
 LANGUAGE sql
...
AS $function$
  WITH token_stats AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE t.level = 3) as l3_count,
      COUNT(*) FILTER (WHERE t.level >= 4) as l3_plus_count
    FROM tokens t
  ...
  SELECT 
    ...
    COALESCE(ts.l3_count, 0) as l3_count,
