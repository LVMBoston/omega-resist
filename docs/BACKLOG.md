...
**Deferred work:**
  ...
  - i. **Partially shipped 2026-06-26** — `ParityHarness.tsx` extended with a new §3 "Plain-text hotspots" section that pairs the editor's CSS rendering against the shared `renderPlainTextSvg` helper. As part of this, the standard-hotspot SVG generation in `render-stats-snapshot/index.ts` was extracted into `supabase/functions/_shared/render/plainText.ts` (canonical) + `src/shared/render/plainText.ts` (re-export). SSR now delegates to that helper; 7 new Vitest cases lock in word-wrap, alignment, padding, XML-escaping, and clipPath behavior. Remaining work: campaign-story segment fixtures (full/first/second), live_number with custom styles, and email/map-legend hotspot fixtures.
