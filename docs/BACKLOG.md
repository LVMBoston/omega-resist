...
**Deferred work:**
  ...
  - c. ~~Collapse the duplicated `campaignStorySplit.ts` (one in `src/lib/`, one in `supabase/functions/render-stats-snapshot/`) into the shared copy.~~ **Shipped 2026-06-26** as `src/shared/render/campaignStorySplit.ts`. `src/lib/campaignStorySplit.ts` is now a thin re-export; SSR imports the shared file via relative path. Test moved to `src/shared/render/campaignStorySplit.test.ts` (7 tests passing).
  - d. ~~Move hotspot default styles (font size, family, weight, color, padding, line-height, text-align, vertical-align — keyed by hotspot type) into `src/shared/render/hotspotDefaults.ts`. Replace inline literals on both sides with imports.~~ **Shipped 2026-06-26** as `supabase/functions/_shared/render/hotspotDefaults.ts` (canonical) + `src/shared/render/hotspotDefaults.ts` (re-export). Editor (`StatsPageSlide.tsx`, `HybridSlide.tsx`) and SSR now call `resolveLiveNumberStyle(style, { isStory })`. Surfaced divergence per step k: SSR's non-story default padding was `0`, editor was `12px` for multi-line values — adopted the editor's behavior (multi-line values get story defaults in both). 7 new Vitest cases lock in defaults.
  - e. ~~Move `textLayout.ts` (word-wrapping, line-height, vertical-alignment math) and `manualHtml.ts` (manual-entry HTML normalization) into the shared module.~~ **Shipped 2026-06-26** — `manualHtml.ts` moved to `supabase/functions/_shared/render/manualHtml.ts`; new `supabase/functions/_shared/render/textLayout.ts` exports `wordWrap`, `maxCharsForWidth`, `normalizeVAlign`, `freeSpaceOffset`, `tspanStartY`, and the `AVG_CHAR_WIDTH_RATIO` / `LINE_HEIGHT_RATIO` constants. SSR now calls these helpers; inlined duplicates removed. Editor re-exports under `src/shared/render/` for the parity harness. 16 new Vitest cases (66 total passing). SSR redeployed.
  - f. ~~Move `mapMarkerRules.ts` (marker color, stroke, spawn-cue rules) into the shared module — last because it has the largest surface and most map-library interaction.~~ **Shipped 2026-06-26** as `supabase/functions/_shared/render/mapMarkerRules.ts` (canonical) + `src/shared/render/mapMarkerRules.ts` (re-export). Both palettes — hex strings for the live editor's SVG markers and `[r,g,b]` tuples for SSR pixel ops — live in one file. `MapHotspotRenderer.tsx` now calls `resolveMarkerFill` / `resolveMarkerStroke`; SSR imports `MEDIUM_COLORS_RGB`, `DEFAULT_COLOR_RGB`, `SPAWN_STROKE_RGB`, `DEFAULT_STROKE_RGB`. 6 new Vitest cases lock in palette sync, fallback behavior, and spawn-stroke flip. SSR redeployed.
  ...
  - i. **Shipped 2026-06-26** — `ParityHarness.tsx` extended with two new sections: §3 "Plain-text hotspots" (7 fixtures) pairing the editor's CSS rendering against the shared `renderPlainTextSvg` helper, and §4 "Campaign-story segment split" (3 fixtures) rendering `full`/`first`/`second` side-by-side so the splitter's pinning rules are visually verifiable. Standard-hotspot SVG generation in `render-stats-snapshot/index.ts` is now delegated to `supabase/functions/_shared/render/plainText.ts` (canonical) + `src/shared/render/plainText.ts` (re-export). 7 Vitest cases lock in word-wrap, alignment, padding, XML-escaping, and clipPath behavior. Remaining work: live_number with custom styles, and email/map-legend hotspot fixtures.
