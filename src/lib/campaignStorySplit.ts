/**
 * Split a campaign_story narrative into two roughly-equal halves at the
...
export function splitCampaignStoryAtMidpoint(story: string): {
  ...
  // Pin title to first, footer to second.
  ...
  // Find split index in `middle` minimizing |leftChars - rightChars|,
  // where leftChars includes the pinned title and rightChars includes
  // the pinned footer.
  ...
  let runningLeft = 0;
  // splitIdx i means: middle[0..i-1] go left, middle[i..] go right. i in [1..middle.length-1]
