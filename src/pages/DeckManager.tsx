...
export default function DeckManager() {
  ...
  const uploadViralImage = async (): Promise<string | null> => {
    ...
    const { data, error } = await supabase.storage
      .from("slides")
      .upload(fileName, viralImageFile);
    ...
    const { data: urlData } = supabase.storage.from("slides").getPublicUrl(data.path);
  ...
  const handleSaveViralSlide = async (finalHotspots: any[]) => {
    ...
    const { error: configError } = await supabase.from("viral_slide_configs").insert({
      slug: viralSlug,
    ...
    const { data: slides, error: slidesError } = await supabase
      .from("slide_items")
      .select("position")
      .eq("deck_slug", selectedDeck)
      .order("position", { ascending: false })
      .limit(1);

    const nextPosition = slides && slides.length > 0 ? slides[0].position + 1 : 1;

    const { error: slideError } = await supabase.from("slide_items").insert({
      deck_slug: selectedDeck,
      type: "spread-word",
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="spread-word"
                    checked={addSpreadWord}
                    onCheckedChange={(checked) => setAddSpreadWord(checked as boolean)}
                  />
                ...
                {addSpreadWord && (
                  <div className="ml-6 space-y-3">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="viral-upload">Upload Viral Slide Image</Label>
                      <Input
                        id="viral-upload"
                        type="file"
                        accept="image/*"
