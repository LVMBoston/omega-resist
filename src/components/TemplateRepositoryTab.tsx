...
export function TemplateRepositoryTab() {
  ...
  const createTemplate = useMutation({
    ...
    onError: (error: Error) => {
      toast({ variant: "destructive", title: "Error", description: `Failed to create template: ${error.message}` });
    },
  });
  ...
  const deleteTemplate = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("viral_slide_configs").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["interactive-templates"] });
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.slug || !formData.image_url) {
      toast({ variant: "destructive", title: "Validation Error", description: "Name, slug, and image are required" });
      return;
    }

    if (formData.template_type === "interactive_share" && (!formData.hotspots || formData.hotspots.length === 0)) {
      toast({ variant: "destructive", title: "Validation Error", description: "Interactive templates must have at least one hotspot" });
      return;
    }
    ...
    if (formData.template_type === "interactive_share" && formData.hotspots.length > 0) {
      ...
      if (overlaps.size > 0) {
        ...
        toast({ variant: "destructive", title: "Cannot Save - Hotspots Overlap", description: `${overlaps.size} hotspot${overlaps.size > 1 ? 's are' : ' is'} overlapping: ${hotspotNames}. Please reposition them so they don't touch.`, duration: 6000 });
        return;
      }
      ...
      if (outOfBoundsIds.length > 0) {
        ...
        toast({ variant: "destructive", title: "Cannot Save - Hotspots Out of Bounds", description: `${outOfBoundsIds.length} hotspot${outOfBoundsIds.length > 1 ? 's extend' : ' extends'} beyond the image boundaries: ${hotspotNames}. Please resize or reposition them to fit within the image.`, duration: 6000 });
        return;
      }
    }
    ...
    if (formData.template_type === "interactive_share" && formData.hotspots.length > 0) {
      try {
        ...
        toast({ title: "Generating thumbnail...", description: "Creating preview with hotspot icons" });
        ...
        toast({ variant: "destructive", title: "Thumbnail generation failed", description: error.message || "Proceeding without thumbnail" });
      } finally {
      ...
      >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    toast({ variant: "destructive", title: "Invalid Interactive Slide", description: "This template has no hotspots configured. Edit it and add at least one hotspot to make it functional." });
                  }}
              ...
              <TooltipContent>Click to see why this template is invalid</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ) : (
          <TooltipProvider>
        ...
        <CardContent>
          ...
          <div className="flex gap-2 justify-center flex-wrap">
            <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); setViewingCampaigns(template.id); }}>
  ...
  return (
    <>
          ...
          <DialogContent className="max-w-[95vw] w-[95vw] h-[95vh] max-h-[95vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input id="name" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g., Spread the Word - Blue Theme" />
              </div>

              <div>
                <Label htmlFor="slug">Slug (unique identifier)</Label>
                <Input id="slug" value={formData.slug} onChange={(e) => setFormData(prev => ({ ...prev, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') }))} placeholder="e.g., spread-word-blue" />
              </div>

              <div>
                <Label htmlFor="description">Description</Label>
                <Textarea id="description" value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} placeholder="Describe this template..." />
              </div>
              ...
              <Collapsible>
                ...
                <CollapsibleContent className="pt-2 space-y-2">
                  ...
                  <div>
                    ...
                    <Input id="image-upload" type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) handleImageUpload(file); }} />
                  </div>
                  {!formData.image_url && (
              ...
              <div className="flex items-center space-x-2">
                <Switch id="default" checked={formData.is_default} onCheckedChange={(checked) => setFormData(prev => ({ ...prev, is_default: checked }))} />
      ...
      <Tabs value={activeFilter} onValueChange={(val) => setActiveFilter(val as "all" | "action" | "data" | "hybrid")}>
        ...
        {isLoading ? (
          ...
          <>
              ...
              {dataTemplates.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold text-green-900 dark:text-green-100">DATA TEMPLATES</h2>
                    <Badge variant="outline" className="ml-2 text-green-600 border-green-300">{dataTemplates.length}</Badge>
                  </div>
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{dataTemplates.map(renderTemplateCard)}</div>
                </div>
              )}
              {actionTemplates.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold text-blue-900 dark:text-blue-100">ACTION TEMPLATES</h2>
                    <Badge variant="outline" className="ml-2 text-blue-600 border-blue-300">{actionTemplates.length}</Badge>
                  </div>
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{actionTemplates.map(renderTemplateCard)}</div>
                </div>
              )}
              {hybridTemplates.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold text-purple-900 dark:text-purple-100">HYBRID TEMPLATES</h2>
                    <Badge variant="outline" className="ml-2 text-purple-600 border-purple-300">{hybridTemplates.length}</Badge>
                  </div>
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{hybridTemplates.map(renderTemplateCard)}</div>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="action">
              {actionTemplates.length === 0 ? (
                <Card><CardContent className="py-12 text-center"><MousePointerClick className="h-12 w-12 mx-auto mb-4 text-blue-400" /><h3 className="text-lg font-semibold mb-2">No Action Templates</h3><p className="text-muted-foreground mb-4">Create your first action template for navigation and sharing</p></CardContent></Card>
              ) : (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{actionTemplates.map(renderTemplateCard)}</div>
              )}
            </TabsContent>
            ...
            <TabsContent value="data">
              {dataTemplates.length === 0 ? (
                <Card><CardContent className="py-12 text-center"><BarChart3 className="h-12 w-12 mx-auto mb-4 text-green-400" /><h3 className="text-lg font-semibold mb-2">No Data Templates</h3><p className="text-muted-foreground mb-4">Create your first data template for live campaign metrics</p></CardContent></Card>
              ) : (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{dataTemplates.map(renderTemplateCard)}</div>
              )}
            </TabsContent>
            ...
            <TabsContent value="hybrid">
              {hybridTemplates.length === 0 ? (
                <Card><CardContent className="py-12 text-center"><Layers className="h-12 w-12 mx-auto mb-4 text-purple-400" /><h3 className="text-lg font-semibold mb-2">No Hybrid Templates</h3><p className="text-muted-foreground mb-4">Promote an action template to hybrid by clicking "Add Data Layer"</p></CardContent></Card>
              ) : (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{hybridTemplates.map(renderTemplateCard)}</div>
              )}
            </TabsContent>
