...
a. After resolving the nearest zip code via `get_nearest_zip_code` RPC, if `city` is still null, query the `zip_codes` table for that zip code's `city` and `state` columns and use them as fallback values for `city` and `region`.

b. This is a ~10-line addition to the existing US-location branch, after the Nominatim parsing and zip lookup are complete.

## 4. Backfill (one-time SQL migration)

a. Run an UPDATE to fill in missing city/region on existing `url_events` rows that have a `zip_code` but null `city`, by joining against the `zip_codes` table:

```sql
UPDATE url_events ue
SET city = zc.city, region = zc.state
FROM zip_codes zc
